                                                  ...
                                                  pj_pool_factory *pf)
{
    ...
    /* Create mutex. */
    status = pj_mutex_create_simple(pool, "ps codec factory",
				    &ps_factory.mutex);
    if (status != PJ_SUCCESS) {
    	goto on_error;
    }
...
PJ_DEF(pj_status_t) pjmedia_codec_ps_vid_deinit(void)
{
    ...
    status = pjmedia_vid_codec_mgr_unregister_factory(ps_factory.mgr,
						      &ps_factory.base);
					     ...
					     pj_bool_t *has_more)
{
    ...
    *has_more = PJ_FALSE;
    ...
    if (ff->whole) {
	status = ps_codec_encode_whole(codec, opt, input, out_size,
					   output);
    } else {
